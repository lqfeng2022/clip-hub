...
const useClipPlaylistManager = (clip: Clip, onClose: () => void) => {
  ...
  //  and it will be updated on the `lists` and `clip.id`
  const initialSelected = useMemo(() => lists
    .filter(list => list.items.some(item => item.video.id === clip.id))
    .map(list => list.id), 
    [lists, clip.id]
  )
  ...
  const handleListItemUpdate = () => {
    ...
    // if we have added list, add clip to the lists
    if (added.length > 0)
      addListItem({ video_id: clip.id, listIds: added })

    // if we have removed list, remove the clip from the lists
    if (removed.length > 0) {
      const deleteTargets = removed.map((listId) => {
        const list = lists.find(l => l.id === listId)
        const item = list?.items.find(i => i.video.id === clip.id)
