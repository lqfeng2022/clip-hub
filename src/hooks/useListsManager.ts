...
import useListItemsDelete from './interact-list/useListItemsDelete'
import useListItemPost from './interact-list/useListItemPost'
import useListPost from './interact-list/useListPost'
import useLists from './interact-list/useLists'
import useListProducts from './interact-list/useListProducts'
import { useParams } from 'react-router-dom'
import useList from './interact-list/useList'
...
const useListsManager = (
  ...
  const { mutate: deleteListItems } = useListItemsDelete()

  
  // get all lists with the product id
  const { data: productLists } = useProductLists(product.id.toString())
  
  // Compute initial selected lists based on **fresh data**
  // `useMemo` cash the reslut to get rid of the expensive computation
  const initialSelected = useMemo(
    () => productLists?.map(list => list.id) ?? [],
    [productLists]
  )
  ...
  useEffect(() => {
    const isBookmarked = initialSelected.length > 0
  ...
  const handleListItemUpdate = () => {
    ...
    if (removed.length > 0) {
      promises.push(
        new Promise(resolve =>
          deleteListItems(
            { product_id: product.id, listIds: removed }, 
            { onSuccess: resolve }
        ))
      )
    }
